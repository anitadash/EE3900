...
%
\author{ Anita Dash %<-this  stops a space
...
\item In the circuit, the switch S is connected to position P for a long time so that the charge on the capacitor
	becomes $q_1 \, \mu C$. Then S is switched to position Q.  After a long time, the charge on the capacitor is
		...
		\begin{figure}[!ht]
			...
			]{/home/anitadash/Documents/cktsig/ckt.jpg}
			\caption{}
			\label{fig:ckt}
\end{figure}
\item Draw the circuit using latex-tikz.\\
\solution The circuit can be found in Fig-\ref{fig:ckt-q1}
\begin{figure}[!h]
...
\end{align}
	\item Now consider the following resistive circuit transformed from 
		...
		\begin{figure}[!ht]
			\centering
			\includegraphics[width=\columnwidth]{/home/anitadash/Documents/cktsig/lap-ckt.jpg}
			\caption{}
			\label{fig:lap-ckt}
\end{figure}
		where 
...
\begin{align}
    ...
    \label{eq:v2-t}
\end{align}
The python code can be found in the link below, and the code plots the following graph
\begin{lstlisting}
wget https://github.com/anitadash/EE3900/blob/main/cktsig/Codes/2_6.py
\end{lstlisting}
\begin{figure}[!htb]
    \includegraphics[width=\columnwidth]{/home/anitadash/Documents/cktsig/Figure_1.png}
    \caption{$v_{C_0}(t)$ after the switch is flipped}
    \label{fig:v2-t}
\end{figure}
	\item Verify your result using ngspice. \\
	\solution The ngspice script that simulates the given circuit can be found in the link below and the generated output is depicted in Fig. \eqref{fig:v2-t}.
	\begin{lstlisting}
wget https://github.com/anitadash/EE3900/blob/main/cktsig/Codes/2_7.cir
\end{lstlisting}
...
The python code can be found in the link below, and the code plots the following graph
\begin{lstlisting}
wget https://github.com/anitadash/EE3900/blob/main/cktsig/Codes/3_4.py
\end{lstlisting}
\begin{figure}[!htb]
    \includegraphics[width=\columnwidth]{/home/anitadash/Documents/cktsig/Figure_2.png}
    \caption{$v_{C_0}(t)$ after the switch is flipped}
    \label{fig:v2-t}
\end{figure}
	\item Verify your result using ngspice.\\
	\solution \solution The ngspice script that simulates the given circuit can be found in the link below and the generated output is depicted in Fig. \eqref{fig:v2-t}.
\begin{lstlisting}
wget https://github.com/anitadash/EE3900/blob/main/cktsig/Codes/3_5.cir
\end{lstlisting}
	\item Find $v_{C_0}(0-), v_{C_0}(0+)$ and  $v_{C_0}(\infty) $. \\
\solution From the initial conditions,
\begin{align}
    v_{C_0}(0-) = \frac{q_1}{C_0} = \frac{4}{3} Volts
\end{align}
Using \eqref{eq:v2-t},
\begin{align}
    v_{C_0}(0+) &= \lim_{t \to 0+}v_{C_0}(t) = \frac{4}{3} Volts \\
    v_{C_0}(\infty) &= \lim_{t \to \infty}v_{C_0}(t) = \frac{2}{3} Volts
\end{align}
...
\begin{align}
    &\dfrac{i_1}{t} = \dfrac{i_2}{t} + \dfrac{i_3}{t} \label{eq:diff1}\\
    &R_1\dfrac{i_1}{t} + \frac{i_2}{C_0} = 0 \label{eq:dfrac2}\\
    &R_2\dfrac{i_3}{t} - \frac{i_2}{C_0} = 0 \label{eq:dfrac3}
\end{align}
Using \eqref{eq:dfrac1} and \eqref{eq:dfrac3} in \eqref{eq:dfrac2},
\begin{align}
    &R_1\brak{\dfrac{i_2}{t} + \dfrac{i_3}{t}} + \frac{i_2}{C_0} = 0 \\
    &R_1\dfrac{i_2}{t} + \brak{1 + \frac{R_1}{R_2}}\frac{i_2}{C_0} = 0 \\
    &\dfrac{i_2}{t} + \brak{\frac{1}{R_1} + \frac{1}{R_2}}\frac{i_2}{C_0} = 0 \\
    &\dfrac{i_2}{t} + \frac{i_2}{\tau} = 0
    \label{eq:dfrac-eqn-init}
\end{align}
where $\tau = \frac{C_0R_1R_2}{R_1 + R_2}$ is the RC time 
constant of the circuit. Note that $i_2(0) = \frac{V_2}{R_2}$ A and 
$i_2 = C_0\dfrac{V}{t}$, where $V$ is the voltage of the capacitor. 
Hence, integrating \eqref{eq:dfrac-eqn-init},
\begin{align}
    C_0\dfrac{V}{t} - \frac{V_2}{R_2} + \frac{C_0V}{\tau} &= 0 \\
    \implies \dfrac{V}{t} + \frac{V}{\tau} = \frac{V_2}{C_0R_2}
    \label{eq:dfrac-eqn}
\end{align}
...
\solution The python code can be found in the link below, and the code plots the following graph
\begin{lstlisting}
wget https://github.com/anitadash/EE3900/blob/main/cktsig/Codes/4_3.py
\end{lstlisting}
\begin{figure}[!ht]
    \includegraphics[width=\columnwidth]{/home/anitadash/Documents/cktsig/Figure_3.png}
    \caption{Plot of $H(s)$.}
    \label{fig:Hs}
\end{figure}
Clearly, $H(s)$ is a low-pass filter.
\item Using trapezoidal rule for integration, formulate the dfracerence
equation by considering 
\begin{align}
	y(n) = y(t)\vert_{t=n}
\end{align}
\solution Integrating \eqref{eq:dfrac-eqn} between limits $n$ to $n+1$ 
...
sampling interval. he python code can be found in the link below, and the code plots the following graph
\begin{lstlisting}
wget https://github.com/anitadash/EE3900/blob/main/cktsig/Codes/4_3.py
\end{lstlisting}
\begin{figure}
    \includegraphics[width=\columnwidth]{/home/anitadash/Documents/cktsig/Figure_4.png}
